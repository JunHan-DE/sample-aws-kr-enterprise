...
4. [Cost Deep Analysis (Athena)](#4-cost-deep-analysis-athena)
...
이 플랫폼은 두 가지 데이터 소스의 **강점에 집중**하여 중복 없이 역할을 분리합니다:

- **Prometheus (AMP)**: 실시간 집계 메트릭 모니터링. 카운터와 비율(rate)을 30초 갱신으로 보여줌. "지금 무슨 일이 일어나고 있는가?"에 답함
- **Athena (S3 + SQL)**: 이벤트 레벨 심층 분석. 개별 API 요청의 레이턴시, 에러 상세, 도구 결과 크기 등을 SQL로 쿼리. "왜 그런 일이 일어났는가?"에 답함

두 데이터 소스 간 중복 패널은 없습니다.
...
| 대시보드 | UID | 패널 수 | 데이터 소스 | 목적 |
|----------|-----|---------|-------------|------|
| **Cost Deep Analysis** | `claude-code-cost` | 10 | Athena | 임계값 색상, 테이블 셀 컬러링, 요청 단위 비용 분석, 모델별 비용 심층 비교 |
