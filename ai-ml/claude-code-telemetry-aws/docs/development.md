...
│   │   ├── cost-analysis.json              # Prometheus+Athena 하이브리드 (10패널, 비용 심층 분석)
...
- 대시보드 JSON 내 `datasource.uid` 값이 Grafana 데이터 소스 UID와 정확히 일치해야 합니다.
  ...
  - 하이브리드 대시보드 (1종): `"uid": "amp"` + `"uid": "athena"` — cost-analysis.json (비용 합산=Prometheus, 상세=Athena)
  - Athena 대시보드 (3종): `"uid": "athena"` — usage-insights.json, tool-analytics.json, api-performance.json
