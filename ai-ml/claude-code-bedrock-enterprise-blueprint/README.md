...
Claude Code on Amazon Bedrock을 엔터프라이즈 환경에서 안전하게 운영하기 위한 레퍼런스 아키텍처입니다. SSO 인증, LLM Gateway, 사용자별 예산 관리, 사용량 모니터링까지 엔드투엔드 인프라를 CDK로 구현합니다.

개발자가 `aws sso login` 한번으로 인증하면, Token Service가 SSO 자격증명을 검증하고 LiteLLM Virtual Key를 자동 생성/반환합니다. Claude Code는 이 Virtual Key로 LLM Gateway(LiteLLM Proxy)를 통해 Amazon Bedrock의 Claude 모델을 호출합니다.
...
| `CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS` | `1` | Beta 헤더/필드 제거 |

## 관련 문서

| 문서 | 설명 |
|------|------|
| [llm-gateway-guide.md](llm-gateway-guide.md) | 전체 배포 가이드 (SSO, 인증 흐름, Gateway, 예산, 모니터링, 보안) |
| [Anthropic - Bedrock 설정](https://docs.anthropic.com/en/docs/claude-code/bedrock) | Claude Code Bedrock 공식 문서 |
| [Anthropic - Enterprise Setup](https://docs.anthropic.com/en/docs/claude-code/enterprise-setup) | Claude Code Enterprise 공식 문서 |
| [LiteLLM - Bedrock Pass-through](https://docs.litellm.ai/docs/pass_through/bedrock) | LiteLLM Bedrock pass-through 문서 |

## 라이선스

이 프로젝트는 사내 인프라 구축을 위한 프로젝트입니다.
